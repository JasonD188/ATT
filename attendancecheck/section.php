...
$sql = "SELECT section_code FROM sections"; 
...
<body class="flex items-center justify-center h-screen bg-cover bg-center" style="background-image: url('pictures/background.jpg');">
    <div class="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
       <div class="mb-4 text-center">
        ...
        <h2 class="text-2xl font-bold mb-4 text-center text-blue-600">SECTION</h2>
        <div id="loadingBar" class="mb-4"></div>
        <div class="space-y-4">
            <?php
  
            foreach ($sections as $section) {
                echo '<button onclick="startLoadingBar(\'indexattendance.html\')" class="w-full bg-blue-500 text-white font-semibold py-2 rounded hover:bg-blue-600 transition duration-200 hover:underline">' . htmlspecialchars($section) . '</button>';
            }
            ?>
        </div>
